...
list<WayPoint> manualWpList;
list<WayPoint> manualNextWpList;
...
State               *egoState;
Planner				*planner;       // A pointer to the planner class
...
void keyboard_cb(unsigned char key, int x, int y)
{
    ...
    {
        planner->itterate(*egoModel);
        planner->expandTree();
        glutPostRedisplay();
    }
    else if(key == ' ')
    {
        setWPstate = WPstateEnum(setWPstate + 1);

        if(setWPstate >= FINISHED)
        {
            setWPstate = SET_POINT;

            WayPoint tempWp;
            tempWp.ID         = manualWpList.size() + 1;
            tempWp.wp         = Point2D(setWP_x, setWP_y);
            tempWp.heading    = setWP_heading;
            double in;
            ...
            cout << "Set type: " << endl;
            cout << "          1: Position constrained" << endl;
            cout << "          2: Heading constrained" << endl;
			cout << "          3: Line constrained" << endl;
			...
			{
			case 1:
				tempWp.type = positionConstrained;
				break;
			case 2:
				tempWp.type = headingConstrained;
				break;
			default:
				tempWp.type = positionConstrained;
				break;
			...
			cout << endl;
            cout << "Set entry behaviour: " << endl;
            cout << "          1: Safety_Behaviour" << endl;
			cout << "          2: Racing_Behaviour" << endl;

			int entry_behaviour;
            cin >> entry_behaviour;
			switch(entry_behaviour)
			{
				case 1: 
					tempWp.entryBehaviour = Safety_Behaviour;
					break;
				case 2: 
					tempWp.entryBehaviour = Racing_Behaviour;
					break;
				default: 
					tempWp.entryBehaviour = Safety_Behaviour;
					break;
			...
			switch(exit_behaviour)
			{
				case 1: 
					tempWp.exitBehaviour = Safety_Behaviour;
					break;
				case 2: 
					tempWp.exitBehaviour = Racing_Behaviour;
					break;
				default: 
					tempWp.exitBehaviour = Safety_Behaviour;
					break;
			...
			if(!manualWpList.empty()){
			manualNextWpList.push_back(manualWpList.front());
			}

            manualWpList.push_back(tempWp);
			
			

			planner->setGlobalReferencePath(manualWpList,manualNextWpList);
        }
...
void makeMenu()
{
    ...
    // Submenu level 1 for "planner mode"
    int id_planner = glutCreateMenu(plannerSubmenu_cb);
    glutAddMenuEntry(" Draw Tree   ",  1);
    glutAddMenuEntry(" Draw Safe   ", 2);
    glutAddMenuEntry(" Draw Trajectories   ", 3);
    glutAddMenuEntry(" Draw Control   ", 4);

    // Submenu level 1 for "planner mode"
    int id_mission = glutCreateMenu(missionControl_cb);
    glutAddMenuEntry(" Reset plan  ",  1);
    glutAddMenuEntry(" Set point manually   ", 2);
    glutAddMenuEntry(" Load plan 1 ", 3);
    glutAddMenuEntry(" Load plan 2  ", 4);
    glutAddMenuEntry(" Load plan 3  ", 5);
    ...
    glutAddSubMenu  (" Draw mode   ", id_planner);
    glutAddSubMenu  (" Mission control   ", id_mission);
...
void missionControl_cb(int id)
{
    if(id == 1)    // if id is valid ( != -1 )
    {
        list<WayPoint> empty;
        planner->setGlobalReferencePath(empty,empty);
    }
    if(id == 2)    // if id is valid ( != -1 )
    {
        RUN = false;

        manualWpList.clear();
        WayPoint egoWp;
        egoWp.wp = Point2D(egoState->x, egoState->y);
        egoWp.type = positionConstrained;
        egoWp.velocity = 10;

        manualWpList.push_back(egoWp);

        state = SETWP;
    }
    if(id == 3)    // if id is valid ( != -1 )
    {
        planner->setGlobalReferencePath(missionHandler->getMissionPlan(1),missionHandler->getNextMissionPlan(1));
    }
    if(id == 4)    // if id is valid ( != -1 )
    {
        planner->setGlobalReferencePath(missionHandler->getMissionPlan(2),missionHandler->getNextMissionPlan(2));
    }
    if(id == 5)    // if id is valid ( != -1 )
    {
        planner->setGlobalReferencePath(missionHandler->getMissionPlan(3),missionHandler->getNextMissionPlan(3));
    }
}

void plannerSubmenu_cb(int id)
{
    if(id == 1)    // if id is valid ( != -1 )
    {
        DRAW_TREE = !DRAW_TREE;
...
void display()
{
    ...
    //draw center of road
    //drawRoadCenter(planner->getRoadCenter(), SCENE_SCALE, drawOffset_x, drawOffset_y);

    //draw waypoints
    GLfloat notVisitedColor[] = {0.0f, 1.0f, 0.0f};
    GLfloat notVisitedColorLine[] = {1.0f, 1.0f, 0.0f};
    drawWPTrajectory(planner->getGlobalReferencePath(), SCENE_SCALE, drawOffset_x, drawOffset_y, notVisitedColor, notVisitedColorLine);

	drawOptimalTrajectorylist(missionHandler->get_optimal_trajectory(),SCENE_SCALE, drawOffset_x, drawOffset_y, notVisitedColor, notVisitedColorLine);

    //draw visited way points
    GLfloat visitedColor[] = {1.0f, 0.0f, 0.0f};
    GLfloat visitedColorLine[] = {1.0f, 0.0f, 0.0f};
    drawWPTrajectory(planner->getVisitedWayPoints(), SCENE_SCALE, drawOffset_x, drawOffset_y, visitedColor, visitedColorLine);


    GLfloat color[] = {1.0f, 0.0f, 0.0f};
    drawCircle(egoModel->controller->lookAheadDistance*SCENE_SCALE, egoState->x*SCENE_SCALE - drawOffset_x*SCENE_SCALE, 0.2, egoState->y*SCENE_SCALE - drawOffset_y*SCENE_SCALE, 10, color);

    //draw look ahead points
    GLfloat frontColor[] = {1.0f, 1.0f, 0.0f};
    drawCircle(0.5, egoModel->controller->lookAheadPoints[0].x*SCENE_SCALE - drawOffset_x*SCENE_SCALE, 0.2, egoModel->controller->lookAheadPoints[0].y*SCENE_SCALE - drawOffset_y*SCENE_SCALE, 10, frontColor);

    GLfloat backColor[] = {1.0f, 0.0f, 1.0f};
    drawCircle(0.5, egoModel->controller->lookAheadPoints[1].x*SCENE_SCALE - drawOffset_x*SCENE_SCALE, 0.2, egoModel->controller->lookAheadPoints[1].y*SCENE_SCALE - drawOffset_y*SCENE_SCALE, 10, backColor);

    //draw the tree
    if(DRAW_TREE)
    {
        drawTreeForward(&planner->forwardNodeList, SCENE_SCALE, drawOffset_x, drawOffset_y, DRAW_CONTROL, DRAW_TRAJ);
        drawTreeReverse(&planner->reverseNodeList, SCENE_SCALE, drawOffset_x, drawOffset_y, DRAW_CONTROL, DRAW_TRAJ);
    }

    //draw safe nodes
    if(DRAW_SAFE)
        drawSafeNodes(&planner->stoppingNodeList,SCENE_SCALE, drawOffset_x, drawOffset_y);

    //draw obstacles
    drawRectObstacle(&obstacleMap->staticObstacles, SCENE_SCALE, drawOffset_x, drawOffset_y);

    if(&planner->getBestNode())
        drawLowCostPath(&planner->forwardNodeList, planner->getBestNode(), egoModel->controller->getLastWp(), SCENE_SCALE, drawOffset_x, drawOffset_y);

	//drawCostMap(lcmMap, *egoState, SCENE_SCALE, drawOffset_x, drawOffset_y);
    ...
    ss.str("");
    ss.clear();
    ss << "Number of nodes: " << planner->numNodes;
    putText(ss.str(), 20, HEIGHT -20, WIDTH, HEIGHT);
    ...
    ss.str("");
    ss.clear();
    ss << "Steer angle: " << (int)(egoState->steerAngle*180/M_PI);
    putText(ss.str(), 20, HEIGHT -100, WIDTH, HEIGHT);

    ss.str("");
    ss.clear();
    switch (planner->getBehaviourState())
    {
    ...
    putText(ss.str(), 20, HEIGHT -120, WIDTH, HEIGHT);
...
void glutVizualisation::setPlanner(Planner* plannerIn)
{
    planner = plannerIn;
}

//void glutVizualisation::setCostMap(mock_lcm::grid_map  *lcmMapIn)
//{
//    lcmMap = lcmMapIn;
//}
...
void glutVizualisation::setMissionHandler(MissionHandler *missionHandler_in)
{
    missionHandler = missionHandler_in;
}
